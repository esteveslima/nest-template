...
import { IParamsRepositoryModifyMedia } from '../../../interfaces/services/database/repositories/media/methods/modify-media.interface';
...
import { IParamsRepositoryRegisterMedia } from '../../../interfaces/services/database/repositories/media/methods/register-media.interface';
import { IParamsRepositorySearchMedia } from '../../../interfaces/services/database/repositories/media/methods/search-media.interface';
...
export class MediaRepository {
  ...
  async incrementMediaViewsById(uuid: string): Promise<boolean> {
    const incrementResult = await this.repository.increment(
      { id: uuid },
      'views',
      1,
    );
  ...
  async modifyMediaById(
    ...
    const mediaModified = await this.repository.update(criteria, media);
